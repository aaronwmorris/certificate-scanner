...
class expired_certificate_scanner(object):
    ...
    def sslyzeScan(self):
        ...
        for scan_result in scanner.get_results():
            ...
            certinfo = scan_result.scan_result.certificate_info
            if certinfo.status == ScanCommandAttemptStatusEnum.ERROR:
                logger.error('Error at %s:%d', scan_entry.host, scan_entry.port)
                #logger.error('Error: %d', scan_result)
            ...
            cert_deployment_0 = certinfo.result.certificate_deployments[0]
    ...
    def report(self, days):
        ...
        output_o = open('{0:s}_results_{1:s}.csv'.format(self.filename, now_str), 'w')
